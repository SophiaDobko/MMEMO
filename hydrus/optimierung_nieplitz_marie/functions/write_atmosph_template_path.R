...
write.atmosph.in = function (project.path, maxAL, deltaT, atm.bc.data, hCritS = 0, 
          input.pet = T, LAI = 0.39, ...) 
{
  out.file = "ATMOSPH.IN"
  template_atmosph_in = paste0(project.path, "/ATMOSPH.IN")
  atm_data = readLines(con = template_atmosph_in, n = -1L, 
                       encoding = "unknown")
  ...
  if (input.pet == TRUE) {
    ...
    atm_data = atm_data[-c(extinction_ind, extinction_ind + 
                             1)]
  }
  ...
  bc_data_vars = c("tAtm", "Prec", "rSoil", "rRoot", "hCritA", 
                   "rB", "hB", "ht")
  bc_data_new = atm.bc.data[1:maxAL, bc_data_vars]
  row.names(bc_data_new) = NULL
  tstep_decimals = get.decimalplaces(deltaT)
  fmt_vec = c("%11.0f", "%12.3f", "%12.4f", "%12.4f", "%12.0f", 
              rep("%12.4f", 8))
  fmt_vec[1] = sub(pattern = "0", replacement = tstep_decimals, 
                   fmt_vec[1])
  bc_data_fmt = bc_data_new
  for (a in 1:nrow(bc_data_fmt)) {
    bc_data_fmt[a, ] = sprintf(fmt = fmt_vec[1:ncol(bc_data_fmt)], 
                               bc_data_new[a, ])
  }
  bc_data_fmt = apply(bc_data_fmt, MARGIN = 1, FUN = paste, 
                      collapse = "")
